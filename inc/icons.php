 ...
 *
 * Uses Heroicons (https://heroicons.com) — same library the Figma template uses.
 *
 * Usage: estatein_icon( 'home', array( 'class' => 'w-6' ) );
 *
 * @package Estatein
 */
...
/**
 * Return the SVG markup for a named icon.
 *
 * @param string $name  Icon name.
 * @param int    $size  Pixel size (square). Default 20.
 * @return string
 */
function estatein_icon( $name, $size = 20 ) {
    ...
    $icons = array(
        ...
        'office'    => '<path stroke-linecap="round" stroke-linejoin="round" d="M2.25 21h19.5m-18-18v18m10.5-18v18m6-13.5V21M6.75 6.75h.75m-.75 3h.75m-.75 3h.75m3-6h.75m-.75 3h.75m-.75 3h.75M6.75 21v-3.375c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21M3 3h12m-.75 4.5H21m-3.75 3.75h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Z"/>',

        'arrow-up-right' => '<path stroke-linecap="round" stroke-linejoin="round" d="M4.5 19.5 19.5 4.5m0 0H8.25m11.25 0v11.25"/>',
        ...
        'bed'       => '<path stroke-linecap="round" stroke-linejoin="round" d="M3 12.75V18m0-5.25h18m-18 0V8.25A2.25 2.25 0 0 1 5.25 6h13.5A2.25 2.25 0 0 1 21 8.25v4.5M3 12.75 21 12.75M21 12.75V18M5.25 9h4.5m4.5 0h4.5"/>',

        'bath'      => '<path stroke-linecap="round" stroke-linejoin="round" d="M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/>',
    ...
    return sprintf(
        '<svg xmlns="http://www.w3.org/2000/svg" width="%1$d" height="%1$d" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">%2$s</svg>',
        intval( $size ),
        $icons[ $name ]
    );
}
 ...
 * Echo helper.
 */
function estatein_the_icon( $name, $size = 20 ) {
    echo estatein_icon( $name, $size ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}
